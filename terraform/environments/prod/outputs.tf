...
output "disaster_recovery_resources" {
  description = "Resources created for disaster recovery"
  value = var.enable_disaster_recovery ? {
    job_bookmarks_replica = aws_dynamodb_table.job_bookmarks_replica[0].name
    job_state_replica     = aws_dynamodb_table.job_state_replica[0].name
  } : null
}
...
output "config_rules" {
  ...
  value = {
    ...
    dynamodb_encryption  = aws_config_config_rule.dynamodb_encryption.name
  }
}
